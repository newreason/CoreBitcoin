...
static const char* LTCBase58Alphabet = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz";

NSMutableData* LTCDataFromBase58(NSString* string) {
    return LTCDataFromBase58CString([string cStringUsingEncoding:NSASCIIStringEncoding]);
}

NSMutableData* LTCDataFromBase58Check(NSString* string) {
    return LTCDataFromBase58CheckCString([string cStringUsingEncoding:NSASCIIStringEncoding]);
}

NSMutableData* LTCDataFromBase58CString(const char* cstring) {
    ...
    for (const char* p = cstring; *p; p++) {
        const char* p1 = strchr(LTCBase58Alphabet, *p);
        if (p1 == NULL) {
            while (isspace(*p))
                p++;
            if (*p != '\0') {
        ...
        BN_set_word(&bnChar, (BN_ULONG)(p1 - LTCBase58Alphabet));
    ...
    int nLeadingZeros = 0;
    for (const char* p = cstring; *p == LTCBase58Alphabet[0]; p++)
        nLeadingZeros++;
...
NSMutableData* LTCDataFromBase58CheckCString(const char* cstring) {
    if (cstring == NULL) return nil;
    
    NSMutableData* result = LTCDataFromBase58CString(cstring);
    size_t length = result.length;
    if (length < 4) {
        return nil;
    }
    NSData* hash = LTCHash256([result subdataWithRange:NSMakeRange(0, length - 4)]);
...
char* LTCBase58CStringWithData(NSData* data) {
    if (!data) return NULL;
    ...
    NSMutableData* tmp = LTCReversedMutableData(data);
    tmp.length += 1;
    ...
    while (BN_cmp(&bn, &bn0) > 0) {
        ...
        [stringData appendBytes:LTCBase58Alphabet + c length:1];
    }
    finish();
    ...
    for (const unsigned char* p = pbegin; p < pend && *p == 0; p++) {
        [stringData appendBytes:LTCBase58Alphabet + 0 length:1];
    }
    
    // Convert little endian std::string to big endian
    LTCDataReverse(stringData);
    
    [stringData appendBytes:"" length:1];
    
    char* r = malloc(stringData.length);
    memcpy(r, stringData.bytes, stringData.length);
    LTCDataClear(stringData);
    return r;
}

// String in Base58 with checksum
char* LTCBase58CheckCStringWithData(NSData* immutabledata) {
    if (!immutabledata) return NULL;
    // add 4-byte hash check to the end
    NSMutableData* data = [immutabledata mutableCopy];
    NSData* checksum = LTCHash256(data);
    [data appendBytes:checksum.bytes length:4];
    char* result = LTCBase58CStringWithData(data);
    LTCDataClear(data);
    return result;
}

NSString* LTCBase58StringWithData(NSData* data) {
    if (!data) return nil;
    char* s = LTCBase58CStringWithData(data);
    id r = [NSString stringWithCString:s encoding:NSASCIIStringEncoding];
    LTCSecureClearCString(s);
    free(s);
    return r;
}


NSString* LTCBase58CheckStringWithData(NSData* data) {
    if (!data) return nil;
    char* s = LTCBase58CheckCStringWithData(data);
    id r = [NSString stringWithCString:s encoding:NSASCIIStringEncoding];
    LTCSecureClearCString(s);
    free(s);
    return r;
