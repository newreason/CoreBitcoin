...
+ (LTCBigNumber*) curveOrder {
    static LTCBigNumber* order;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        order = [[LTCBigNumber alloc] initWithUnsignedBigEndian:LTCDataWithHexCString("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141")];
    });
    return order;
}
...
- (id) initEmpty {
    if (self = [super init]) {
        ...
        if (!_group) {
            NSLog(@"LTCCurvePoint: EC_GROUP_new_by_curve_name(NID_secp256k1) failed");
            goto finish;
        }
        
        _point = EC_POINT_new(_group);
        if (!_point) {
            NSLog(@"LTCCurvePoint: EC_POINT_new(_group) failed");
            goto finish;
        }
        
        _bnctx = BN_CTX_new();
        if (!_bnctx) {
            NSLog(@"LTCCurvePoint: BN_CTX_new() failed");
            goto finish;
        }
...
- (instancetype) add:(LTCCurvePoint*)otherPoint {
...
- (LTCBigNumber*) x {
    ...
    LTCBigNumber* result = [[LTCBigNumber alloc] initWithBIGNUM:bn];
    BN_CTX_end(_bnctx);
    return result;
}
...
- (id) copyWithZone:(NSZone *)zone {
    return [[LTCCurvePoint alloc] initWithEC_POINT:_point];
}

- (BOOL) isEqual:(LTCCurvePoint*)otherPoint {
...
- (NSString*) description {
    return [NSString stringWithFormat:@"<LTCCurvePoint:0x%p %@>", self, LTCHexFromData(self.data)];
}
