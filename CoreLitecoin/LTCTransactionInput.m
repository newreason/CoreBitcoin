...
@interface LTCTransactionInput ()
@end

static const uint32_t LTCInvalidIndex = 0xFFFFFFFF; // aka "(unsigned int) -1" in BitcoinQT.
static const uint32_t LTCMaxSequence = 0xFFFFFFFF;


@implementation LTCTransactionInput

- (id) init {
    if (self = [super init]) {
        _previousHash = LTCZero256();
        _previousIndex = LTCInvalidIndex;
        _signatureScript = [[LTCScript alloc] init];
        _sequence = LTCMaxSequence; // max
        _value = -1;
    }
    return self;
}
...
- (id) initWithDictionary:(NSDictionary*)dictionary {
    if (self = [self init]) {
        ...
        if (prevHashString) _previousHash = LTCReversedData(LTCDataFromHex(prevHashString));
        ...
        if (dictionary[@"coinbase"]) {
            _coinbaseData = LTCDataFromHex(dictionary[@"coinbase"]);
        } else {
            ...
            if ([scriptSig isKindOfClass:[NSString class]]) {
                _signatureScript = [[LTCScript alloc] initWithString:scriptSig];
            } else if ([scriptSig isKindOfClass:[NSDictionary class]]) {
                if (scriptSig[@"hex"]) {
                    _signatureScript = [[LTCScript alloc] initWithData:LTCDataFromHex(scriptSig[@"hex"])];
                } else if (scriptSig[@"asm"]) {
                    _signatureScript = [[LTCScript alloc] initWithString:scriptSig[@"asm"]];
                }
            }
        }
...
- (NSData*) computePayload {
    ...
    if (self.isCoinbase) {
        [payload appendData:[LTCProtocolSerialization dataForVarInt:self.coinbaseData.length]];
...
- (LTCOutpoint*) outpoint {
    return [[LTCOutpoint alloc] initWithHash:self.previousHash index:self.previousIndex];
}

- (void) setOutpoint:(LTCOutpoint *)outpoint {
    self.previousHash = outpoint.txHash;
    self.previousIndex = outpoint.index;
}

- (NSString*) previousTransactionID {
    return LTCIDFromHash(self.previousHash);
}

- (void) setPreviousTransactionID:(NSString *)previousTransactionID {
    self.previousHash = LTCHashFromID(previousTransactionID);
}
...
- (NSDictionary*) dictionary {
    ...
    dict[@"prev_out"] = @{
                        @"hash": LTCHexFromData(LTCReversedData(_previousHash)), // transaction hashes are reversed
                        @"n": @(_previousIndex),
                        };
    
    if ([self isCoinbase]) {
        dict[@"coinbase"] = LTCHexFromData(_coinbaseData);
    } else {
...
- (BOOL) parseStream:(NSInputStream*)stream {
    ...
    if ([self isCoinbase]) {
        ...
        _signatureScript = [[LTCScript alloc] initWithData:scriptdata];
    }
...
- (BOOL) isCoinbase {
    return (_previousIndex == LTCInvalidIndex) &&
            _previousHash.length == 32 &&
            0 == memcmp(LTCZeroString256(), _previousHash.bytes, 32);
}
