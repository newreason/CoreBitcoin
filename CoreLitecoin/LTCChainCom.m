#import "LTCChainCom.h"
#import "LTCAddress.h"
#import "LTCTransactionOutput.h"
#import "LTCScript.h"
#import "LTCData.h"

@interface LTCChainCom()
@property NSString* token;
@end

@implementation LTCChainCom

// Initalizes a LTCChainCom object with a free API Token from http://chain.com
...
- (NSArray*) unspentOutputsForResponseData:(NSData*)responseData error:(NSError**)errorOut {
    ...
    for (NSDictionary* item in array) {
        ...
        txout.script = [[LTCScript alloc] initWithString:item[@"script"]];
        txout.index = [item[@"output_index"] intValue];
        txout.transactionHash = (LTCReversedData(LTCDataFromHex(item[@"transaction_hash"])));
...
- (NSArray*) unspentOutputsWithAddress:(LTCAddress*)address error:(NSError**)errorOut {
...
- (NSMutableURLRequest*) requestForTransactionBroadcastWithData:(NSData*)data {
    ...
    NSDictionary *requestDictionary = @{@"hex":LTCHexFromData(data)};
