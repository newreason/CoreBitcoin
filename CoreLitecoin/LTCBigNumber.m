...
#define LTCBigNumberCompare(a, b) (BN_cmp(&(a->_bignum), &(b->_bignum)))

@implementation LTCBigNumber {
    @package
    BIGNUM _bignum;
...
- (void) throwIfImmutable {
    if (_immutable) {
        @throw [NSException exceptionWithName:@"Immutable LTCBigNumber is modified" reason:@"" userInfo:nil];
    }
}
...
- (LTCBigNumber*) copyWithZone:(NSZone *)zone {
    LTCBigNumber* to = [[LTCBigNumber alloc] init];
...
- (LTCMutableBigNumber*) mutableCopyWithZone:(NSZone *)zone {
    LTCMutableBigNumber* to = [[LTCMutableBigNumber alloc] init];
    if (BN_copy(&(to->_bignum), &_bignum)) {
        return to;
    }
    return nil;
...
- (BOOL) isEqual:(LTCBigNumber*)other {
    if (![other isKindOfClass:[LTCBigNumber class]]) return NO;
    return LTCBigNumberCompare(self, other) == NSOrderedSame;
}

- (NSComparisonResult)compare:(LTCBigNumber *)other {
    return LTCBigNumberCompare(self, other);
}
...
- (LTCBigNumber*) min:(LTCBigNumber*)other {
    return (LTCBigNumberCompare(self, other) <= 0) ? self : other;
}
...
- (BOOL) less:(LTCBigNumber *)other           { return LTCBigNumberCompare(self, other) <  0; }
- (BOOL) lessOrEqual:(LTCBigNumber *)other    { return LTCBigNumberCompare(self, other) <= 0; }
- (BOOL) greater:(LTCBigNumber *)other        { return LTCBigNumberCompare(self, other) >  0; }
- (BOOL) greaterOrEqual:(LTCBigNumber *)other { return LTCBigNumberCompare(self, other) >= 0; }
    ...
    if (BN_cmp(&bn, &bn0) == 0) {
        ...
        while (BN_cmp(&bn, &bn0) > 0) {
            if (!BN_div(&dv, &rem, &bn, &bnBase, pctx)) {
                NSLog(@"LTCBigNumber: stringInBase failed to BN_div");
                break;
            }
            BN_copy(&bn, &dv);
...
{
    BN_CTX* pctx = BN_CTX_new();
    LTCBigNumber* r = [LTCBigNumber new];
    LTCBigNumber* m = [LTCBigNumber new];
...
@implementation LTCMutableBigNumber
...
+ (instancetype) zero {
    return [[LTCBigNumber zero] mutableCopy];
}

+ (instancetype) one {
    return [[LTCBigNumber one] mutableCopy];
}

+ (instancetype) negativeOne {
    return [[LTCBigNumber negativeOne] mutableCopy];
}
...
- (instancetype) rshift:(unsigned int)shift { // >>=
    ...
    LTCMutableBigNumber* a = [LTCMutableBigNumber one];
    [a lshift:shift];
...
- (instancetype) exp:(LTCBigNumber*)power { // pow(self, p)
    BN_CTX* pctx = BN_CTX_new();
...
- (instancetype) exp:(LTCBigNumber*)power mod:(LTCBigNumber *)mod { // pow(self,p) % m
    BN_CTX* pctx = BN_CTX_new();
