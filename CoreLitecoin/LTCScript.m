...
+ (LTCScriptChunk*) parseChunkFromData:(NSData*)data offset:(NSUInteger)offset;
...
- (id) initWithData:(NSData*)data {
    if (self = [super init]) {
        // It's important to keep around original data to correctly identify the size of the script for LTC_MAX_SCRIPT_SIZE check
...
- (id) initWithAddress:(LTCAddress*)address {
    ...
    if ([address isKindOfClass:[LTCPublicKeyAddress class]]) {
        ...
        LTCOpcode prefix[] = {LTC_OP_DUP, LTC_OP_HASH160};
        ...
        LTCOpcode suffix[] = {LTC_OP_EQUALVERIFY, LTC_OP_CHECKSIG};
        [resultData appendBytes:suffix length:sizeof(suffix)];
        
        return [self initWithData:resultData];
    } else if ([address isKindOfClass:[LTCScriptHashAddress class]]) {
        // OP_HASH160 <hash> OP_EQUAL
        NSMutableData* resultData = [NSMutableData data];
        
        LTCOpcode prefix[] = {LTC_OP_HASH160};
        [resultData appendBytes:prefix length:sizeof(prefix)];
...
- (id) initWithPublicKeys:(NSArray*)publicKeys signaturesRequired:(NSUInteger)signaturesRequired {
    ...
    LTCOpcode m_opcode = LTCOpcodeForSmallInteger(signaturesRequired);
    LTCOpcode n_opcode = LTCOpcodeForSmallInteger(publicKeys.count);
    if (m_opcode == LTC_OP_INVALIDOPCODE) return nil;
    if (n_opcode == LTC_OP_INVALIDOPCODE) return nil;
    ...
    for (NSData* pubkey in publicKeys) {
        NSData* d = [LTCScriptChunk scriptDataForPushdata:pubkey preferredLengthEncoding:-1];
    ...
    LTCOpcode checkmultisig_opcode = LTC_OP_CHECKMULTISIG;
...
- (NSData*) data {
    if (!_data) {
        ...
        for (LTCScriptChunk* chunk in _chunks) {
...
- (NSMutableArray*) parseData:(NSData*)data {
    ...
    while (i < length) {
        LTCScriptChunk* chunk = [LTCScriptChunk parseChunkFromData:data offset:i];
    ...
    while (![scanner isAtEnd]) {
        ...
        // 2. Arbitrary hex pushdata in square brackets (bitcoinj style)
            ...
            if ([scanner scanUpToString:@"]" intoString:&hexstring]) {
                ...
                NSData* data = LTCDataFromHex(hexstring);
        ...
        // 6. Big pushdata in hex
            ...
            if (!mustBePushdata) {
                LTCOpcode opcode = LTCOpcodeForName(word);
                
                // Opcode may be used without OP_ prefix.
                if (opcode == LTC_OP_INVALIDOPCODE) opcode = LTCOpcodeForName([@"OP_" stringByAppendingString:word]);

                // 4. Opcode with and without OP_ prefix
                if (opcode != LTC_OP_INVALIDOPCODE) {
                    ...
                    if ([[NSString stringWithFormat:@"%lld", decimalInteger] isEqualToString:word]) {
                        ...
                        if (decimalInteger >= -1 && decimalInteger <= 16) {
                            opcode = LTCOpcodeForSmallInteger((NSUInteger)decimalInteger);
                            
                            if (opcode == LTC_OP_INVALIDOPCODE) {
                                @throw [NSException exceptionWithName:@"LTCScript parser inconsistency"
                            ...
                            LTCBigNumber* bn = [[LTCBigNumber alloc] initWithInt64:decimalInteger];
                            [scriptData appendData:[LTCScriptChunk scriptDataForPushdata:bn.signedLittleEndian preferredLengthEncoding:-1]];
                            continue;
                        }
                    }
            ...
            NSData* data = LTCDataFromHex(word);
        ...
        if (pushdata) {
            NSData* addedScriptData = [LTCScriptChunk scriptDataForPushdata:pushdata preferredLengthEncoding:lengthEncoding];
...
- (BOOL) isPublicKeyScript {
    ...
    return [self pushdataAtIndex:0].length > 1
        && [self opcodeAtIndex:1] == LTC_OP_CHECKSIG;
}
...
- (BOOL) isPayToPublicKeyHashScript {
    ...
    LTCScriptChunk* dataChunk = [self chunkAtIndex:2];
    
    return [self opcodeAtIndex:0] == LTC_OP_DUP
        && [self opcodeAtIndex:1] == LTC_OP_HASH160
        && !dataChunk.isOpcode
        && dataChunk.range.length == 21
        && [self opcodeAtIndex:3] == LTC_OP_EQUALVERIFY
        && [self opcodeAtIndex:4] == LTC_OP_CHECKSIG;
}
...
- (BOOL) isPayToScriptHashScript {
    ...
    LTCScriptChunk* dataChunk = [self chunkAtIndex:1];
    
    return [self opcodeAtIndex:0] == LTC_OP_HASH160
        && !dataChunk.isOpcode
        && dataChunk.range.length == 21          // this is enough to match the exact byte template, any other encoding will be larger.
        && [self opcodeAtIndex:2] == LTC_OP_EQUAL;
}

// Returns YES if the script ends with P2SH check.
// Not used in CoreLitecoin. Similar code is used in bitcoin-ruby. I don't know if we'll ever need it.
- (BOOL) endsWithPayToScriptHash {
    if (_chunks.count < 3) return NO;
    
    return [self opcodeAtIndex:-3] == LTC_OP_HASH160
        && [self pushdataAtIndex:-2].length == 20
        && [self opcodeAtIndex:-1] == LTC_OP_EQUAL;
}
...
- (void) detectMultisigScript {
    ...
    if ([self opcodeAtIndex:-1] != LTC_OP_CHECKMULTISIG) return;
    
    LTCOpcode m_opcode = [self opcodeAtIndex:0];
    LTCOpcode n_opcode = [self opcodeAtIndex:-2];
    
    NSInteger m = LTCSmallIntegerFromOpcode(m_opcode);
    NSInteger n = LTCSmallIntegerFromOpcode(n_opcode);
...
- (void) enumerateOperations:(void(^)(NSUInteger opIndex, LTCOpcode opcode, NSData* pushdata, BOOL* stop))block {
    if (!block) return;
    
    NSUInteger opIndex = 0;
    for (LTCScriptChunk* chunk in _chunks) {
        if (chunk.isOpcode) {
            LTCOpcode opcode = chunk.opcode;
            BOOL stop = NO;
            ...
            BOOL stop = NO;
            block(opIndex, LTC_OP_INVALIDOPCODE, data, &stop);
            if (stop) return;
        }
        opIndex++;
    }
...
- (LTCAddress*) standardAddress {
    if ([self isPayToPublicKeyHashScript]) {
        ...
        if (!dataChunk.isOpcode && dataChunk.range.length == 21) {
            return [LTCPublicKeyAddress addressWithData:dataChunk.pushdata];
        }
        ...
        if (!dataChunk.isOpcode && dataChunk.range.length == 21) {
            return [LTCScriptHashAddress addressWithData:dataChunk.pushdata];
        }
    }
    return nil;
}
...
- (LTCScript*) scriptHashScript {
    return [[LTCScript alloc] initWithAddress:[self scriptHashAddress]];
}

// Returns LTCScriptHashAddress that hashes this script.
// Equivalent to [[script scriptHashScript] standardAddress] or [LTCScriptHashAddress addressWithData:LTCHash160(script.data)]
- (LTCScriptHashAddress*) scriptHashAddress {
    return [LTCScriptHashAddress addressWithData:LTCHash160(self.data)];
}

- (LTCScriptHashAddressTestnet*) scriptHashAddressTestnet {
    return [LTCScriptHashAddressTestnet addressWithData:LTCHash160(self.data)];
}
...
- (LTCScript*) simulatedSignatureScriptWithOptions:(LTCScriptSimulationOptions)opts {
    if ([self isPayToPublicKeyHashScript]) {
        LTCScript* script = [LTCScript new];
        [script appendData:[LTCScript simulatedSignatureWithHashType:LTC_SIGHASH_ALL]];
        [script appendData:(opts & LTCScriptSimulationCompressedPublicKeys) ? [LTCScript simulatedCompressedPubkey] : [LTCScript simulatedUncompressedPubkey]];
        return script;
    } else if ([self isPublicKeyScript]) {
        return [[LTCScript new] appendData:[LTCScript simulatedSignatureWithHashType:LTC_SIGHASH_ALL]];
    } else if ([self isPayToScriptHashScript]) {
        // Check if allowed to simulate input for P2SH as 2-of-3 multisig.
        if (opts & LTCScriptSimulationMultisigP2SH) {
            // This is a wild approximation, but works well if most p2sh scripts are 2-of-3 multisig scripts.
            LTCScript* script = [LTCScript new];
            [script appendOpcode:LTC_OP_0];
            [script appendData:[LTCScript simulatedSignatureWithHashType:LTC_SIGHASH_ALL]];
            [script appendData:[LTCScript simulatedSignatureWithHashType:LTC_SIGHASH_ALL]];
            [script appendData:[LTCScript simulatedMultisigScriptWithSignaturesCount:2 pubkeysCount:3
                                          compressedPubkeys:(opts & LTCScriptSimulationCompressedPublicKeys)].data];
            return script;
        } else {
...
+ (NSData*) simulatedSignatureWithHashType:(LTCSignatureHashType)hashtype {
...
+ (LTCScript*) simulatedMultisigScriptWithSignaturesCount:(NSInteger)m pubkeysCount:(NSInteger)n compressedPubkeys:(BOOL)compressedPubkeys {
    /*
     Script.new <<
     ...
     OP_CHECKMULTISIG
     */
    LTCScript* multisig = [LTCScript new];
    [multisig appendOpcode:LTCOpcodeForSmallInteger(m)];
    NSData* pubkey = compressedPubkeys ? [LTCScript simulatedCompressedPubkey] : [LTCScript simulatedUncompressedPubkey];
    for (NSInteger i = 0; i < n; i++) {
        [multisig appendData:pubkey];
    }
    [multisig appendOpcode:LTCOpcodeForSmallInteger(n)];
    return multisig;
}
...
- (LTCScript*) appendOpcode:(LTCOpcode)opcode {
    NSMutableData* scriptData = [self.data mutableCopy] ?: [NSMutableData data];
    
    [scriptData appendBytes:&opcode length:sizeof(opcode)];
    
    LTCScriptChunk* chunk = [[LTCScriptChunk alloc] init];
    ...
    for (LTCScriptChunk* chunk in _chunks) chunk.scriptData = scriptData;
...
- (LTCScript*) appendData:(NSData*)data {
    if (data.length == 0) return self;
    
    NSMutableData* scriptData = [self.data mutableCopy] ?: [NSMutableData data];
    
    NSData* addedScriptData = [LTCScriptChunk scriptDataForPushdata:data preferredLengthEncoding:-1];
    [scriptData appendData:addedScriptData];
...
- (LTCScript*) appendScript:(LTCScript*)otherScript {
    ...
    for (LTCScriptChunk* chunk in otherScript->_chunks) {
        LTCScriptChunk* chunk2 = [[LTCScriptChunk alloc] init];
...
- (LTCScript*) deleteOccurrencesOfData:(NSData*)data {
    if (data.length == 0) return self;
...
- (LTCScript*) deleteOccurrencesOfOpcode:(LTCOpcode)opcode {
    NSMutableData* md = [NSMutableData data];
...
- (LTCScript*) subScriptFromIndex:(NSUInteger)index {
    NSMutableData* md = [NSMutableData data];
    for (LTCScriptChunk* chunk in [_chunks subarrayWithRange:NSMakeRange(index, _chunks.count - index)]) {
        [md appendData:chunk.chunkData];
    }
    return [[LTCScript alloc] initWithData:md];
}

- (LTCScript*) subScriptToIndex:(NSUInteger)index {
    NSMutableData* md = [NSMutableData data];
    for (LTCScriptChunk* chunk in [_chunks subarrayWithRange:NSMakeRange(0, index)]) {
        [md appendData:chunk.chunkData];
    }
    return [[LTCScript alloc] initWithData:md];
}
...
- (id) copyWithZone:(NSZone *)zone {
    return [[LTCScript alloc] initWithData:self.data];
}
...
- (LTCScriptChunk*) chunkAtIndex:(NSInteger)index {
    LTCScriptChunk* chunk = _chunks[index < 0 ? (_chunks.count + index) : index];
    return chunk;
}
...
- (LTCOpcode) opcodeAtIndex:(NSInteger)index {
    LTCScriptChunk* chunk = _chunks[index < 0 ? (_chunks.count + index) : index];
...
- (LTCBigNumber*) bignumberAtIndex:(NSInteger)index {
    NSData* data = [self pushdataAtIndex:index];
    if (!data) return nil;
    LTCBigNumber* bn = [[LTCBigNumber alloc] initWithSignedLittleEndian:data];
    return bn;
}
...
#pragma mark - Canonical checks (Deprecated methods - use LTCKey API)


+ (BOOL) isCanonicalPublicKey:(NSData*)data error:(NSError**)errorOut {
    return [LTCKey isCanonicalPublicKey:data error:errorOut];
}

+ (BOOL) isCanonicalSignature:(NSData*)data verifyEvenS:(BOOL)verifyEvenS error:(NSError**)errorOut {
    return [LTCKey isCanonicalSignatureWithHashType:data verifyEvenS:verifyEvenS error:errorOut];
}
...
@implementation LTCScriptChunk {
}

- (LTCOpcode) opcode {
    return (LTCOpcode)((const unsigned char*)_scriptData.bytes)[_range.location];
}

- (BOOL) isOpcode {
    LTCOpcode opcode = [self opcode];
    // Pushdata opcodes are not considered a single "opcode".
    // Attention: OP_0 is also "pushdata" code that pushes empty data.
    if (opcode <= LTC_OP_PUSHDATA4) return NO;
    return YES;
}
...
- (NSData*) pushdata {
    ...
    LTCOpcode opcode = [self opcode];
    NSUInteger loc = 1;
    if (opcode == LTC_OP_PUSHDATA1) {
        loc += 1;
    } else if (opcode == LTC_OP_PUSHDATA2) {
        loc += 2;
    } else if (opcode == LTC_OP_PUSHDATA4) {
        loc += 4;
    }
...
- (BOOL) isDataCompact {
    ...
    if (opcode < LTC_OP_PUSHDATA1) return YES; // length fits in one byte under OP_PUSHDATA1.
    if (opcode == LTC_OP_PUSHDATA1) return data.length >= LTC_OP_PUSHDATA1; // length should be less than OP_PUSHDATA1
    if (opcode == LTC_OP_PUSHDATA2) return data.length > 0xff; // length should not fit in one byte
    if (opcode == LTC_OP_PUSHDATA4) return data.length > 0xffff; // length should not fit in two bytes
    return NO;
}
...
- (NSString*) string {
    ...
    if (self.isOpcode) {
        if (opcode == LTC_OP_0) return @"OP_0";
        if (opcode == LTC_OP_1NEGATE) return @"OP_1NEGATE";
        if (opcode >= LTC_OP_1 && opcode <= LTC_OP_16) {
            return [NSString stringWithFormat:@"OP_%u", ((int)opcode + 1 - (int)LTC_OP_1)];
        } else {
            return LTCNameForOpcode(opcode);
        }
    } else {
        NSData* data = [self pushdata];
        ...
        if (![self isDataCompact]) {
            int prefix = 1;
            if (opcode == LTC_OP_PUSHDATA2) prefix = 2;
            if (opcode == LTC_OP_PUSHDATA4) prefix = 4;
...
+ (NSData*) scriptDataForPushdata:(NSData*)data preferredLengthEncoding:(int)preferredLengthEncoding {
    ...
    if (data.length < LTC_OP_PUSHDATA1 && preferredLengthEncoding <= 0) {
...
+ (LTCScriptChunk*) parseChunkFromData:(NSData*)scriptData offset:(NSUInteger)offset {
    ...
    LTCOpcode opcode = bytes[offset];
    
    if (opcode <= LTC_OP_PUSHDATA4) {
        // push data opcode
        ...
        } else if (opcode == LTC_OP_PUSHDATA1) {
            uint8_t dataLength;
