...
#import "LTCErrors.h"
#import "LTCFancyEncryptedMessage.h"
#import "LTCData.h"
#import "LTCKey.h"
...
#define LTCFancyEncryptedMessageChecksumLength 16

static uint8_t LTCEMCompactTargetForFullTarget(uint32_t fullTarget);
static uint32_t LTCEMFullTargetForCompactTarget(uint8_t compactTarget);

@implementation LTCFancyEncryptedMessage {
...
- (id) initWithData:(NSData*)data {
    ...
    if (self = [super init]) {
        ...
        _addressLength = LTCFancyEncryptedMessageAddressLengthNone;
        _decryptedData = data;
    }
    return self;
}

- (NSData*) encryptedDataWithKey:(LTCKey*)recipientKey seed:(NSData*)seed0 {
    ...
    self.address = LTCHash160(recipientPubKey);
    ...
    if (seed0) {
        ...
        void* randomBytes = LTCCreateRandomBytesOfLength(32);
        CC_SHA256_Update(&ctx256, randomBytes, 32);
        LTCSecureMemset(randomBytes, 0, 32);
        free(randomBytes);
    }
    ...
    LTCSecureMemset(digest256, 0, CC_SHA256_DIGEST_LENGTH);
    LTCDataClear(recipientPubKey);
    
    if (self.timestampVariance > 0) {
        NSData* varianceHash = LTCSHA256(seed); // extra hashing to not leak our seed that we'll use for private key nonce.
        uint32_t rand = *((uint32_t*)varianceHash.bytes);
        self.timestamp -= (rand % self.timestampVariance);
    }
    
    uint8_t compactTarget = LTCEMCompactTargetForFullTarget(_difficultyTarget);
    ...
    do {
        ...
        LTCDataClear(messageData); // clear previous trial
        
        [messageData setLength:0];
        
        [messageData appendBytes:LTCFancyEncryptedMessageVersion length:4];
        ...
        LTCKey* nonceKey = [[LTCKey alloc] initWithPrivateKey:privkey];
        ...
        LTCBigNumber* pk = [[LTCBigNumber alloc] initWithUnsignedBigEndian:privkey];
        
        LTCCurvePoint* curvePoint = recipientKey.curvePoint;
        ...
        if (cryptstatus != kCCSuccess) {
            LTCDataClear(encryptedData);
            LTCSecureMemset(&ctx256, 0, sizeof(ctx256));
            encryptedData = nil;
        } else {
        ...
        if ([pointX isKindOfClass:[NSMutableData class]]) {
            LTCDataClear((NSMutableData*)pointX);
        }
        [pk clear];
        [curvePoint clear];
        LTCSecureMemset(&ctx256, 0, sizeof(ctx256));
        ...
        [messageData appendData:[LTCProtocolSerialization dataForVarString:encryptedData]];
        ...
        [messageData appendBytes:digest256 length:LTCFancyEncryptedMessageChecksumLength];
...
- (id) initWithEncryptedData:(NSData*)data {
    if (self = [super init]) {
        ...
        if (datalength < (4 + 1 + 1 + 4 + 1 + 1 + 32 + 1 + LTCFancyEncryptedMessageChecksumLength)) return nil;
        ...
        if (memcmp(LTCFancyEncryptedMessageVersion, msgbytes, sizeof(LTCFancyEncryptedMessageVersion)) != 0) {
            return nil;
        }
        ...
        uint32_t target = LTCEMFullTargetForCompactTarget(msgbytes[4]);
        ...
        LTCKey* nonceKey = [[LTCKey alloc] initWithPublicKey:pubkeyNonceData];
        ...
        _encryptedData = [LTCProtocolSerialization readVarStringFromData:[data subdataWithRange:NSMakeRange(offset, datalength - offset)] readBytes:&encodedMessageLength];
        
        if (_encryptedData == nil) return nil;
        
        offset += encodedMessageLength;
        
        if (datalength < offset + LTCFancyEncryptedMessageChecksumLength) return nil;
        
        _checksum = [data subdataWithRange:NSMakeRange(offset, LTCFancyEncryptedMessageChecksumLength)];
...
- (NSData*) decryptedDataWithKey:(LTCKey*)key error:(NSError**)errorOut {
    ...
    LTCBigNumber* pk = [[LTCBigNumber alloc] initWithUnsignedBigEndian:key.privateKey];
    
    LTCCurvePoint* curvePoint = _nonceKey.curvePoint;
    ...
    if (_checksum.length != LTCFancyEncryptedMessageChecksumLength) {
        LTCSecureMemset(digest256, 0, CC_SHA256_DIGEST_LENGTH);
        return nil;
    }
    
    if (memcmp(digest256, _checksum.bytes, LTCFancyEncryptedMessageChecksumLength) != 0) {
        LTCSecureMemset(digest256, 0, CC_SHA256_DIGEST_LENGTH);
        return nil;
    }
...
static uint8_t LTCEMCompactTargetForFullTarget(uint32_t fullTarget) {
    ...
    for (uint8_t ct = 0xFF; ct >= 0; --ct) {
        if (LTCEMFullTargetForCompactTarget(ct) <= fullTarget) {
            uint32_t order = ct >> 3;
