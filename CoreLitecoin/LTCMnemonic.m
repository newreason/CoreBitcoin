...
#import "LTCMnemonic.h"
#import "LTCData.h"
#import "LTCKeychain.h"
#import "LTCProtocolSerialization.h"
#include <CommonCrypto/CommonKeyDerivation.h>

@interface LTCMnemonic ()

@property(nonatomic, readwrite) LTCMnemonicWordListType wordListType;
...
@property(nonatomic, readwrite) LTCKeychain* keychain;

@end

static inline BOOL LTCMnemonicIsBitSet(uint8_t* buf, int bitIndex) {
...
static inline void LTCMnemonicSetBit(uint8_t* buf, int bitIndex) {
    int value = ((int) buf[bitIndex / 8]) & 0xFF;
    value = value | (1 << (7 - (bitIndex % 8)));
    buf[bitIndex / 8] = (uint8_t) value;
}

static inline void LTCMnemonicIntegerTo11Bits(uint8_t* buf, int bitIndex, int integer) {
    for (int i = 0; i < 11; i++) {
        if ((integer & 0x400) == 0x400) {
            LTCMnemonicSetBit(buf, bitIndex + i);
        }
        integer = integer << 1;
    }
}

static inline NSUInteger LTCMnemonicIntegerFrom11Bits(uint8_t* buf, int bitIndex) {
    NSUInteger value = 0;
    for (int i = 0; i < 11; i++) {
        if (LTCMnemonicIsBitSet(buf, bitIndex + i)) {
            value = (value << 1) | 0x01;
        } else {
            value = (value << 1);
        }
...
@implementation LTCMnemonic


- (id) initWithEntropy:(NSData*)entropy password:(NSString*)password wordListType:(LTCMnemonicWordListType)wordListType {
    if (!entropy) return nil;
    if (wordListType == LTCMnemonicWordListTypeUnknown) return nil;
...
- (id) initWithWords:(NSArray*)words password:(NSString*)password wordListType:(LTCMnemonicWordListType)wordListType {
    if (!words) return nil;
    ...
    if (self = [super init]) {
        ...
        if (wordListType != LTCMnemonicWordListTypeUnknown) {
...
- (id) initWithData:(NSData*)data {
    ...
    if (self = [super init]) {
        NSAssert(sizeof(LTCMnemonicWordListType) == 1, @"must be 1 byte long");

        if (data.length < sizeof(LTCMnemonicWordListType)) {
            // Not enough bytes.
            return nil;
        }
        ...
        _wordListType = ((LTCMnemonicWordListType*)data.bytes)[offset];

        offset += sizeof(LTCMnemonicWordListType);

        if (_wordListType != LTCMnemonicWordListTypeEnglish) {
            // Not supported list.
            return nil;
        }

        NSUInteger encodedEntropyLength = 0;
        _entropy = [LTCProtocolSerialization
        ...
        NSData* passwordData = [LTCProtocolSerialization
                                readVarStringFromData:[data subdataWithRange:NSMakeRange(offset, data.length - offset)]
        ...
        if (!passwordData) {
            ...
            NSLog(@"ERROR: LTCMnemonic cannot decode password (if missing, password should be empty string with 0x00 length prefix).");
            return nil;
        }
        ...
        if (!_password) {
            ...
            NSLog(@"ERROR: LTCMnemonic cannot convert password back to UTF8 string.");
            return nil;
        }

        // If we have some more data, try to read seed from there.
        if (data.length > offset) {
            _seed = [LTCProtocolSerialization
                     readVarStringFromData:[data subdataWithRange:NSMakeRange(offset, data.length - offset)]
                     readBytes:NULL];

            if (!_seed) {
                NSLog(@"ERROR: LTCMnemonic failed to read encoded seed from mnemonic payload. Will recompute it lazily when needed.");
            }
        }
    }
    return self;
...
- (LTCKeychain*) keychain
{
    if (!_keychain) {
        _keychain = [[LTCKeychain alloc] initWithSeed:self.seed];
    }
    return _keychain;
}
...
- (NSData*) dataWithSeed:(BOOL)withSeed {
    ...
    if (_wordListType == LTCMnemonicWordListTypeUnknown) return nil;

    [data appendBytes:&_wordListType length:sizeof(_wordListType)];

    if (!_entropy) return nil;

    [data appendData:[LTCProtocolSerialization dataForVarString:_entropy]];

    NSData* passwordData = [_password ?: @"" dataUsingEncoding:NSUTF8StringEncoding];

    [data appendData:[LTCProtocolSerialization dataForVarString:passwordData]];

    if (withSeed) {
        NSData* seed = self.seed;

        [data appendData:[LTCProtocolSerialization dataForVarString:seed]];
    }
...
- (BOOL) isEqual:(LTCMnemonic*)other {
...
- (NSArray*) wordsForEntropy:(NSData*)entropy wordListType:(LTCMnemonicWordListType)listType {
    if (!entropy) return nil;
    ...
    NSData* checksumHash = LTCSHA256(entropy);
    ...
    for (int i = 0; i < wordsCount; i++) {
        NSUInteger wordIndex = LTCMnemonicIntegerFrom11Bits((uint8_t*)buf.bytes, i * 11);
...
- (NSData*) entropyFromWords:(NSArray*)words wordListType:(LTCMnemonicWordListType)listType {
    if (listType == LTCMnemonicWordListTypeUnknown) return nil;
    if (!words) return nil;
    ...
    for (int i = 0; i < words.count; i++) {
        ...
        LTCMnemonicIntegerTo11Bits((uint8_t*)buf.mutableBytes, i * 11, (int)wordIndex);
    }
...
+ (NSArray*) wordListForType:(LTCMnemonicWordListType)type {
    if (type == LTCMnemonicWordListTypeEnglish) {
