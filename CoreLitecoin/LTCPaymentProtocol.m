...
static NSString* const LTCBitcoinPaymentRequestMimeType = @"application/bitcoin-paymentrequest";
static NSString* const LTCOpenAssetsPaymentRequestMimeType = @"application/oa-paymentrequest";
static NSString* const LTCOpenAssetsPaymentMethodRequestMimeType = @"application/oa-paymentmethodrequest";

@interface LTCPaymentProtocol ()
@property(nonnull, nonatomic, readwrite) NSArray* assetTypes;
@property(nonnull, nonatomic) NSArray* paymentRequestMediaTypes;
@end

@implementation LTCPaymentProtocol

// Instantiates default BIP70 protocol that supports only Bitcoin.
- (nonnull id) init {
    return [self initWithAssetTypes:@[ LTCAssetTypeBitcoin ]];
}
...
- (NSArray*) paymentRequestMediaTypes {
    if (!_paymentRequestMediaTypes && self.assetTypes) {
        ...
        for (NSString* assetType in self.assetTypes) {
            if ([assetType isEqual:LTCAssetTypeBitcoin]) {
                [arr addObject:LTCBitcoinPaymentRequestMimeType];
            } else if ([assetType isEqual:LTCAssetTypeOpenAssets]) {
                [arr addObject:LTCOpenAssetsPaymentRequestMimeType];
            }
        }
...
- (void) loadPaymentMethodRequestFromURL:(nonnull NSURL*)paymentMethodRequestURL
                       completionHandler:(nonnull void(^)(LTCPaymentMethodRequest* __nullable pmr, LTCPaymentRequest* __nullable pr, NSError* __nullable error))completionHandler {
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_DEFAULT, 0), ^{
        ...
        LTCPaymentRequest* pr = ([prOrPmr isKindOfClass:[LTCPaymentRequest class]] ? prOrPmr : nil);
        LTCPaymentMethodRequest* pmr = ([prOrPmr isKindOfClass:[LTCPaymentMethodRequest class]] ? prOrPmr : nil);
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void) loadPaymentRequestFromURL:(nonnull NSURL*)paymentRequestURL completionHandler:(nonnull void(^)(LTCPaymentRequest* __nullable pr, NSError* __nullable error))completionHandler {
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_DEFAULT, 0), ^{
        ...
        LTCPaymentRequest* pr = [self paymentRequestFromData:data response:response error:&error];
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (void) postPayment:(nonnull LTCPayment*)payment URL:(nonnull NSURL*)paymentURL completionHandler:(nonnull void(^)(LTCPaymentACK* __nullable ack, NSError* __nullable error))completionHandler {
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_DEFAULT, 0), ^{
        ...
        LTCPaymentACK* ack = [self paymentACKFromData:data response:response error:&error];
        dispatch_async(dispatch_get_main_queue(), ^{
...
- (nullable NSURLRequest*) requestForPaymentMethodRequestWithURL:(nonnull NSURL*)url timeout:(NSTimeInterval)timeout {
    ...
    [request addValue:LTCBitcoinPaymentRequestMimeType forHTTPHeaderField:@"Accept"];
    [request addValue:LTCOpenAssetsPaymentRequestMimeType forHTTPHeaderField:@"Accept"];
    [request addValue:LTCOpenAssetsPaymentMethodRequestMimeType forHTTPHeaderField:@"Accept"];
    return request;
}
...
- (nullable id) polymorphicPaymentRequestFromData:(nonnull NSData*)data response:(nonnull NSURLResponse*)response error:(NSError* __nullable * __nullable)errorOut {
    ...
    BOOL isPaymentRequest = [mime isEqual:LTCBitcoinPaymentRequestMimeType] ||
                            [mime isEqual:LTCOpenAssetsPaymentRequestMimeType];
    BOOL isPaymentMethodRequest = [mime isEqual:LTCOpenAssetsPaymentMethodRequestMimeType];

    if (!isPaymentRequest && !isPaymentMethodRequest) {
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorPaymentRequestInvalidResponse userInfo:@{}];
        return nil;
    }
    if (data.length > [self maxDataLength]) {
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorPaymentRequestTooBig userInfo:@{}];
        return nil;
    }
    if (isPaymentRequest) {
        LTCPaymentRequest* pr = [[LTCPaymentRequest alloc] initWithData:data];
        if (!pr) {
            if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorPaymentRequestInvalidResponse userInfo:@{}];
            return nil;
        }
        ...
        LTCPaymentMethodRequest* pmr = [[LTCPaymentMethodRequest alloc] initWithData:data];
        if (!pmr) {
            if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorPaymentRequestInvalidResponse userInfo:@{}];
            return nil;
        }
...
- (LTCPaymentRequest*) paymentRequestFromData:(NSData*)data response:(NSURLResponse*)response error:(NSError**)errorOut {
    ...
    if (pr.version == LTCPaymentRequestVersion1 && ![self.assetTypes containsObject:LTCAssetTypeBitcoin]) {
        // Client did not want bitcoin, but received bitcoin.
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorPaymentRequestInvalidResponse userInfo:@{}];
        return nil;
    }
    if (pr.version == LTCPaymentRequestVersionOpenAssets1 && ![self.assetTypes containsObject:LTCAssetTypeOpenAssets]) {
...
- (NSURLRequest*) requestForPayment:(LTCPayment*)payment url:(NSURL*)paymentURL {
    return [self requestForPayment:payment url:paymentURL timeout:10];
}

- (NSURLRequest*) requestForPayment:(LTCPayment*)payment url:(NSURL*)paymentURL timeout:(NSTimeInterval)timeout {
...
- (LTCPaymentACK*) paymentACKFromData:(NSData*)data response:(NSURLResponse*)response error:(NSError**)errorOut {
    ...
    LTCPaymentACK* ack = [[LTCPaymentACK alloc] initWithData:data];
...
+ (void) loadPaymentRequestFromURL:(NSURL*)paymentRequestURL completionHandler:(void(^)(LTCPaymentRequest* pr, NSError* error))completionHandler {
    [[[self alloc] init] loadPaymentRequestFromURL:paymentRequestURL completionHandler:completionHandler];
}
+ (void) postPayment:(LTCPayment*)payment URL:(NSURL*)paymentURL completionHandler:(void(^)(LTCPaymentACK* ack, NSError* error))completionHandler {
