...
- (id) init {
    if (self = [super init]) {
        // init default values
        _version = LTCBlockCurrentVersion;
        _previousBlockHash = LTCZero256();
        _merkleRootHash = LTCZero256();
        _time = 0;
        _difficultyTarget = 0;
        _nonce = 0;
...
- (NSString*) previousBlockID {
    return LTCIDFromHash(self.previousBlockHash);
}

- (void) setPreviousBlockID:(NSString *)previousBlockID {
    self.previousBlockHash = LTCHashFromID(previousBlockID) ?: LTCZero256();
}
...
- (NSData*) computePayload {
    ...
    [data appendData:self.previousBlockHash ?: LTCZero256()];
    [data appendData:self.merkleRootHash ?: LTCZero256()];
...
- (id) copyWithZone:(NSZone *)zone {
    LTCBlockHeader* bh = [[LTCBlockHeader alloc] init];
