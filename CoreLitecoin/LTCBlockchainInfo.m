#import "LTCBlockchainInfo.h"
#import "LTCTransactionOutput.h"
#import "LTCScript.h"
#import "LTCData.h"

@implementation LTCBlockchainInfo
...
// List of LTCTransactionOutput instances.
- (NSArray*) unspentOutputsForResponseData:(NSData*)responseData error:(NSError**)errorOut {
    ...
    for (NSDictionary* item in dict[@"unspent_outputs"]) {
        ...
        txout.script = [[LTCScript alloc] initWithData:LTCDataFromHex(item[@"script"])];
        txout.index = [item[@"tx_output_n"] intValue];
        txout.confirmations = [item[@"confirmations"] unsignedIntegerValue];
        txout.transactionHash = (LTCDataFromHex(item[@"tx_hash"])); // unlike many other APIs, here tx_hash is not reversed, but a raw hash in hex.
...
- (NSMutableURLRequest*) requestForTransactionBroadcastWithData:(NSData*)data {
    ...
    NSString* form = [NSString stringWithFormat:@"tx=%@", LTCHexFromData(data)];
