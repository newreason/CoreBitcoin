...
#define CHECK_IF_CLEARED if (_cleared) { [[NSException exceptionWithName:@"LTCKey: instance was already cleared." reason:@"" userInfo:nil] raise]; }

#define LTCCompressedPubkeyLength   (33)
#define LTCUncompressedPubkeyLength (65)

static BOOL    LTCKeyCheckPrivateKeyRange(const unsigned char *secret, size_t length);
static BOOL    LTCKeyCheckSignatureElement(const unsigned char *bytes, int length, BOOL half);
static int     LTCRegenerateKey(EC_KEY *eckey, BIGNUM *priv_key);
static NSData* LTCSignatureHashForBinaryMessage(NSData* data);
static int     ECDSA_SIG_recover_key_GFp(EC_KEY *eckey, ECDSA_SIG *ecsig, const unsigned char *msg, int msglen, int recid, int check);

@interface LTCKey ()
@end

@implementation LTCKey {
    BOOL _cleared;
    EC_KEY* _key;
...
- (id) initWithCurvePoint:(LTCCurvePoint*)curvePoint {
    if (self = [super init]) {
...
- (id) initWithWIF:(NSString*)wifString {
    LTCPrivateKeyAddress* addr = [LTCPrivateKeyAddress addressWithString:wifString];
    if (![addr isKindOfClass:[LTCPrivateKeyAddress class]]) {
        return nil;
    }
...
- (NSData*)signatureForHash:(NSData*)hash hashType:(LTCSignatureHashType)hashType {
   return [self signatureForHash:hash appendHashType:YES hashType:hashType];
}

- (NSData*)signatureForHash:(NSData*)hash withHashType:(LTCSignatureHashType)hashType {
    return [self signatureForHash:hash appendHashType:YES hashType:hashType];
}

- (NSData*)signatureForHash:(NSData*)hash appendHashType:(BOOL)appendHashType hashType:(LTCSignatureHashType)hashType {
    CHECK_IF_CLEARED;
    ...
    LTCMutableBigNumber* privkeyBN = [[LTCMutableBigNumber alloc] initWithBIGNUM:privkeyBIGNUM];
    LTCBigNumber* n = [LTCCurvePoint curveOrder];

    NSMutableData* kdata = [self signatureNonceForHash:hash];
    LTCMutableBigNumber* k = [[LTCMutableBigNumber alloc] initWithUnsignedBigEndian:kdata];
    [k mod:n]; // make sure k belongs to [0, n - 1]

    LTCDataClear(kdata);

    LTCCurvePoint* K = [[LTCCurvePoint generator] multiply:k];
    LTCBigNumber* Kx = K.x;

    LTCBigNumber* hashBN = [[LTCBigNumber alloc] initWithUnsignedBigEndian:hash];

    // Compute s = (k^-1)*(h + Kx*privkey)

    LTCBigNumber* signatureBN = [[[privkeyBN multiply:Kx mod:n] add:hashBN mod:n] multiply:[k inverseMod:n] mod:n];
...
- (NSMutableData*) signatureNonceForHash:(NSData*)hash {
    ...
    LTCMutableBigNumber* hashModOrder = [[[LTCMutableBigNumber alloc] initWithUnsignedBigEndian:hash] mod:order];
    ...
    for (int i = 0; i < 10000; i++) {
        ...
        LTCBigNumber* bn = [[LTCBigNumber alloc] initWithUnsignedBigEndian:[NSData dataWithBytesNoCopy:t length:sizeof(t) freeWhenDone:NO]];
...
- (LTCCurvePoint*) curvePoint {
    CHECK_IF_CLEARED;
    const EC_POINT* ecpoint = EC_KEY_get0_public_key(_key);
    LTCCurvePoint* cp = [[LTCCurvePoint alloc] initWithEC_POINT:ecpoint];
    return cp;
}
...
- (void) setPublicKey:(NSData *)publicKey {
    ...
    _publicKeyCompressed = ([self lengthOfPubKey:_publicKey] == LTCCompressedPubkeyLength);
...
- (void) setDERPrivateKey:(NSData *)DERPrivateKey {
    ...
    LTCDataClear(_publicKey); _publicKey = nil;
    [self prepareKeyIfNeeded];
...
- (void) generateKeyPair {
    ...
    do {
        RAND_bytes(bytes, 32);
    } while (!LTCKeyCheckPrivateKeyRange(bytes, 32));
...
- (id) copy {
    CHECK_IF_CLEARED;
    LTCKey* newKey = [[LTCKey alloc] initWithNewKeyPair:NO];
    if (_key) newKey->_key = EC_KEY_dup(_key);
    return newKey;
}

- (BOOL) isEqual:(LTCKey*)otherKey {
    CHECK_IF_CLEARED;
...
- (NSString*) description {
    return [NSString stringWithFormat:@"<LTCKey:0x%p %@>", self, LTCHexFromData(self.publicKeyCached)];
}

- (NSString*) debugDescription
{
    return [NSString stringWithFormat:@"<LTCKey:0x%p pubkey:%@ privkey:%@>", self, LTCHexFromData(self.publicKeyCached), LTCHexFromData(self.privateKey)];
}
...
- (NSUInteger) lengthOfPubKey:(NSData*)data {
    ...
    if (header == 2 || header == 3)
        return LTCCompressedPubkeyLength;
    if (header == 4 || header == 6 || header == 7)
        return LTCUncompressedPubkeyLength;
    return 0;
}
...
#pragma mark - LTCAddress Import/Export




- (id) initWithPrivateKeyAddress:(LTCPrivateKeyAddress*)privateKeyAddress {
...
- (LTCPublicKeyAddress*) publicKeyAddress {
    CHECK_IF_CLEARED;
    NSData* pubkey = [self publicKeyCached];
    if (pubkey.length == 0) return nil;
    return [LTCPublicKeyAddress addressWithData:LTCHash160(pubkey)];
}

- (LTCPublicKeyAddress*) address {
    CHECK_IF_CLEARED;
    NSData* pubkey = [self publicKeyCached];
    if (pubkey.length == 0) return nil;
    return [LTCPublicKeyAddress addressWithData:LTCHash160(pubkey)];
...
- (LTCPublicKeyAddressTestnet*) addressTestnet {
    CHECK_IF_CLEARED;
    NSData* pubkey = [self publicKeyCached];
    if (pubkey.length == 0) return nil;
    return [LTCPublicKeyAddressTestnet addressWithData:LTCHash160(pubkey)];
}

- (LTCPublicKeyAddress*) compressedPublicKeyAddress {
    CHECK_IF_CLEARED;
...
- (LTCPublicKeyAddress*) uncompressedPublicKeyAddress {
    CHECK_IF_CLEARED;
...
- (LTCPrivateKeyAddress*) privateKeyAddress {
    CHECK_IF_CLEARED;
    NSMutableData* privkey = self.privateKey;
    ...
    LTCDataClear(privkey);
    return result;
}


- (LTCPrivateKeyAddressTestnet*) privateKeyAddressTestnet {
    CHECK_IF_CLEARED;
    NSMutableData* privkey = self.privateKey;
    if (privkey.length == 0) return nil;

    LTCPrivateKeyAddressTestnet* result = [LTCPrivateKeyAddressTestnet addressWithData:privkey publicKeyCompressed:self.isPublicKeyCompressed];
    LTCDataClear(privkey);
    return result;
}
...
- (NSData*) compactSignatureForHash:(NSData*)hash {
    ...
    if (nBitsR <= 256 && nBitsS <= 256) {
        ...
        for (int i=0; i < 4; i++) {
            ...
            LTCKey* key2 = [[LTCKey alloc] initWithNewKeyPair:NO];
...
+ (LTCKey*) verifyCompactSignature:(NSData*)compactSignature forHash:(NSData*)hash {
    ...
    // It will be updated via direct access to _key ivar.
    LTCKey* key = [[LTCKey alloc] initWithNewKeyPair:NO];
...
- (BOOL) isValidCompactSignature:(NSData*)signature forHash:(NSData*)hash {
    CHECK_IF_CLEARED;
    LTCKey* key = [[self class] verifyCompactSignature:signature forHash:hash];
    return [key isEqual:self];
}


// Multiplies a public key of the receiver with a given private key and returns resulting curve point as LTCKey object (pubkey only).
// Pubkey compression flag is the same as on receiver.
- (LTCKey*) diffieHellmanWithPrivateKey:(LTCKey*)privkey {

    LTCCurvePoint* curvePoint = self.curvePoint;
    LTCBigNumber* pk = [[LTCBigNumber alloc] initWithUnsignedBigEndian:privkey.privateKey];
    ...
    LTCKey* dhKey = [[LTCKey alloc] initWithCurvePoint:curvePoint];
...
- (NSData*) signatureForBinaryMessage:(NSData*)data {
    if (!data) return nil;
    return [self compactSignatureForHash:LTCSignatureHashForBinaryMessage(data)];
}

// Verifies message against given signature. On success returns a public key.
+ (LTCKey*) verifySignature:(NSData*)signature forMessage:(NSString*)message {
    return [self verifySignature:signature forBinaryMessage:[message dataUsingEncoding:NSASCIIStringEncoding]];
}

+ (LTCKey*) verifySignature:(NSData*)signature forBinaryMessage:(NSData *)data {
    if (!signature || !data) return nil;
    return [self verifyCompactSignature:signature forHash:LTCSignatureHashForBinaryMessage(data)];
}
...
- (BOOL) isValidSignature:(NSData*)signature forBinaryMessage:(NSData *)data {
    LTCKey* key = [[self class] verifySignature:signature forBinaryMessage:data];
    return [key isEqual:self];
}
...
+ (BOOL) isCanonicalPublicKey:(NSData*)data error:(NSError**)errorOut {
    ...
    if (length < 33) {
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorNonCanonicalPublicKey userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical public key: too short.", @"")}];
        return NO;
    }
    ...
    if (bytes[0] == 0x04) {
        ...
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorNonCanonicalPublicKey userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical public key: length of uncompressed key must be 65 bytes.", @"")}];
        ...
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorNonCanonicalPublicKey userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical public key: length of compressed key must be 33 bytes.", @"")}];
        
        return NO;
    }
    
    // Unknown public key format.
    if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorNonCanonicalPublicKey userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Unknown non-canonical public key.", @"")}];
...
+ (BOOL) isCanonicalSignatureWithHashType:(NSData*)data verifyLowerS:(BOOL)verifyLowerS error:(NSError**)errorOut {
    ...
    if (length < 9) {
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: too short.", @"")}];
        return NO;
    }
    
    // Non-canonical signature: too long
    if (length > 73) {
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: too long.", @"")}];
        return NO;
    }
    
    unsigned char nHashType = bytes[length - 1] & (~(LTC_SIGHASH_ANYONECANPAY));
    
    if (nHashType < LTC_SIGHASH_ALL || nHashType > LTC_SIGHASH_SINGLE) {
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: unknown hashtype byte.", @"")}];
        return NO;
    }
    
    if (bytes[0] != 0x30) {
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: wrong type.", @"")}];
        return NO;
    }
    
    if (bytes[1] != length-3) {
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: wrong length marker.", @"")}];
        return NO;
    }
    ...
    if (5 + lenR >= length) {
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: S length misplaced.", @"")}];
        return NO;
    }
    ...
    if ((unsigned long)(lenR+lenS+7) != length) {
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: R+S length mismatch", @"")}];
        return NO;
    }
    
    const unsigned char *R = &bytes[4];
    if (R[-2] != 0x02) {
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: R value type mismatch", @"")}];
        return NO;
    }
    if (lenR == 0) {
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: R length is zero", @"")}];
        return NO;
    }
    
    if (R[0] & 0x80) {
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: R value negative", @"")}];
        return NO;
    }
    
    if (lenR > 1 && (R[0] == 0x00) && !(R[1] & 0x80)) {
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: R value excessively padded", @"")}];
        return NO;
    }
    
    const unsigned char *S = &bytes[6+lenR];
    if (S[-2] != 0x02) {
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: S value type mismatch", @"")}];
        return NO;
    }
    
    if (lenS == 0) {
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: S length is zero", @"")}];
        return NO;
    }
    
    if (S[0] & 0x80) {
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: S value is negative", @"")}];
        return NO;
    }
    
    if (lenS > 1 && (S[0] == 0x00) && !(S[1] & 0x80)) {
        if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain code:LTCErrorNonCanonicalScriptSignature userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"Non-canonical signature: S value excessively padded", @"")}];
        return NO;
    }
    
    if (verifyLowerS) {
        if (!LTCKeyCheckSignatureElement(S, lenS, YES)) {
            if (errorOut) *errorOut = [NSError errorWithDomain:LTCErrorDomain
                                                          code:LTCErrorNonCanonicalScriptSignature
...
static const unsigned char LTCKeyMaxModOrder[32] = {
...
static const unsigned char LTCKeyMaxModHalfOrder[32] = {
...
static const unsigned char LTCKeyZero[0] = {};

NSComparisonResult LTCKeyCompareBigEndian(const unsigned char *c1, size_t c1len,
...
static BOOL LTCKeyCheckPrivateKeyRange(const unsigned char *secret, size_t length) {
    return LTCKeyCompareBigEndian(secret, length, LTCKeyZero, 0) > 0 &&
           LTCKeyCompareBigEndian(secret, length, LTCKeyMaxModOrder, 32) <= 0;
}

static BOOL LTCKeyCheckSignatureElement(const unsigned char *bytes, int length, BOOL half) {
    return LTCKeyCompareBigEndian(bytes, length, LTCKeyZero, 0) > 0 &&
           LTCKeyCompareBigEndian(bytes, length, half ? LTCKeyMaxModHalfOrder : LTCKeyMaxModOrder, 32) <= 0;
}

static NSData* LTCSignatureHashForBinaryMessage(NSData* msg) {
    NSMutableData* data = [NSMutableData data];
    [data appendData:[LTCProtocolSerialization dataForVarString:[@"Bitcoin Signed Message:\n" dataUsingEncoding:NSASCIIStringEncoding]]];
    [data appendData:[LTCProtocolSerialization dataForVarString:msg ?: [NSData data]]];
    return LTCHash256(data);
}
