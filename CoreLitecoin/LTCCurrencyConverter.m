#import "LTCCurrencyConverter.h"
#import "LTCNumberFormatter.h"

@implementation LTCCurrencyConverter

- (id) init {
    if (self = [super init]) {
        _mode = LTCCurrencyConverterModeAverage;
    }
    return self;
}
...
- (LTCAmount) bitcoinFromFiat:(NSDecimalNumber*)fiatAmount {
    switch (_mode) {
        case LTCCurrencyConverterModeAverage:
            if ([self isZero:_averageRate]) return 0;
            return LTCAmountFromDecimalNumber([[fiatAmount decimalNumberByDividingBy:_averageRate] decimalNumberByMultiplyingByPowerOf10:8]);

        case LTCCurrencyConverterModeBuy:
            if ([self isZero:_buyRate]) return 0;
            return LTCAmountFromDecimalNumber([[fiatAmount decimalNumberByDividingBy:_buyRate] decimalNumberByMultiplyingByPowerOf10:8]);

        case LTCCurrencyConverterModeSell:
            if ([self isZero:_sellRate]) return 0;
            return LTCAmountFromDecimalNumber([[fiatAmount decimalNumberByDividingBy:_sellRate] decimalNumberByMultiplyingByPowerOf10:8]);

            // TODO: add order book modes

        default:
            [[NSException exceptionWithName:@"LTCCurrencyConverter Not supported Mode" reason:@"This mode is not supported yet" userInfo:nil] raise];
            break;
    }
...
- (NSDecimalNumber*) fiatFromBitcoin:(LTCAmount)satoshis {
    switch (_mode) {
        case LTCCurrencyConverterModeAverage:
...
- (id) copyWithZone:(NSZone *)zone {
    return [[LTCCurrencyConverter alloc] initWithDictionary:self.dictionary];
}
