...
#import "LTCEncryptedMessage.h"
#import <CommonCrypto/CommonCrypto.h>

@implementation LTCEncryptedMessage

- (NSData*) encrypt:(NSData*)plaintext {

    NSData* privkey = self.senderKey.privateKey;
    NSData* iv = [LTCHMACSHA256(privkey, plaintext) subdataWithRange:NSMakeRange(0, 16)];

    LTCBigNumber* r = [[LTCBigNumber alloc] initWithUnsignedBigEndian:privkey];
    NSData* Rbuf = self.senderKey.compressedPublicKey;
    LTCCurvePoint* cp = self.recipientKey.curvePoint;
    LTCCurvePoint* P = [cp multiply:r];
    LTCBigNumber* S = P.x;
    NSData* Sbuf = S.unsignedBigEndian; // ensures padding to 32 bytes
    NSData* kEkM = LTCSHA512(Sbuf);
    ...
    NSData* d = LTCHMACSHA256(kM, ivct);
...
- (NSData*) decrypt:(NSData*)ciphertext {
    ...
    LTCBigNumber* kB = [[LTCBigNumber alloc] initWithUnsignedBigEndian:privkey];

    self.senderKey = [[LTCKey alloc] initWithPublicKey:[ciphertext subdataWithRange:NSMakeRange(0, 33)]];
    LTCCurvePoint* R = self.senderKey.curvePoint;
    LTCCurvePoint* P = [R multiply:kB];
    LTCBigNumber* S = P.x;
    NSData* Sbuf = S.unsignedBigEndian; // ensures padding to 32 bytes
    NSData* kEkM = LTCSHA512(Sbuf);
    NSData* kE = [kEkM subdataWithRange:NSMakeRange(0, 32)];
    ...
    NSData* d2 = LTCHMACSHA256(kM, ivct);
